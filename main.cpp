...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	*wvpData = MakeIdentity4x4();
	...
	//auto transformtionMatirxResourceSprite =
	//	dxCommon->CreateBufferResource(sizeof(Matrix4x4));
	////データを書き込む
	...
	while (true)
	{
		...
		if (winApp->ProcessMessage())
		{
			...
			*wvpData = worldViewProjectionMatrix;
			...
			//barrier.Transition.pResource = dxCommon->swapChainResources(backBufferIndex);
			...
			////描画先のRTVを設定する
			//dxCommon->GetSwapChain()->CreateSwapChain()->OMSetRenderTargets(1, &rtvHandles[backBufferIndex], false, nullptr);
			////指定した色で画面全体をクリアする
			//float clearColor[] = { 0.1f,0.25f,0.5f,1.0f };
			//dxCommon->GetSwapChain()->ClearRenderTargetView(rtvHandles[backBufferIndex], clearColor, 0, nullptr);
